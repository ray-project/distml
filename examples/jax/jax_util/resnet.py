...
def ConvBlock(kernel_size, filters, strides=(2, 2)):
    ...
    return stax.serial(
        FanOut(2), stax.parallel(Main, Shortcut), FanInSum, Relu)
...
def IdentityBlock(kernel_size, filters):
    ...
    return stax.serial(
        FanOut(2), stax.parallel(Main, Identity), FanInSum, Relu)
...
def ResNet101(num_classes):
    return stax.serial(
        ...
        IdentityBlock(3, [512, 512]), AvgPool((7, 7), padding="SAME"), Flatten,
...
def ResNet18(num_classes):
    return stax.serial(
        ...
        BasicBlock(3, [256, 256]), IdentityBlock(3, [256, 256]),
        BasicBlock(3, [512, 512]), IdentityBlock(3, [512, 512]),
        AvgPool((7, 7), padding="SAME"), Flatten, Dense(num_classes),
        LogSoftmax)


def MLP(num_classes):
    return stax.serial(Flatten, Dense(32), BatchNorm(), Relu, Dense(128),
                       BatchNorm(), Relu, Dense(num_classes), LogSoftmax)
