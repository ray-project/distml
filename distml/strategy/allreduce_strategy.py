...
from .util import ThroughputCollection, func_timer
...
class AllReduceStrategy(BaseTrainer):
    def __init__(self,
        ...
        # TODO(HUI): This is not for release.
        self._collector = ThroughputCollection(batch_size=kwargs["record_config"]["batch_size"],
                                               save_freq=kwargs["record_config"]["save_freq"],
                                               job_name=kwargs["record_config"]["job_name"],)
        # if self._global_batch_size:
        #     self._collector = ThroughputCollection(batch_size=self._global_batch_size)
        # else:
    ...
    def validate(self,
        ...
        return batch_metrics  # validate results should be the same in all workers
...
class Replica:
    ...
    def make_iterator(self, training=True):
        ...
        if training:
            self.training_iterator = iter(self.training_operator._get_train_loader())
        else:
            self.validation_iterator = iter(self.training_operator._get_validation_loader())
    ...
    def get_data_loader_len(self, training=True):
        ...
        if hasattr(loader, "__len__"):
            ...
            raise RuntimeError("Data loader has no attribute `__len__`. "
...
class DataParallelGroup:
    ...
    def _make_iterator(self, training):
        return [replica.make_iterator.remote(training)
    ...
    def get_data_loader_len(self, training=True):
        """Return the number of batches in the data loader."""
        lens = ray.get([replica.get_data_loader_len.remote(training=training)
                        for replica in self.replicas])
        if len(set(lens)) != 1:
            # TODO(Hao): is this correct after we add distributed data loader?
            raise RuntimeError("All replica should have the same dataloader len.")
        return lens[0]
    ...
    def validate_batch(self):
        rets = [replica.validate_batch.remote()
