...
class ParameterServerStrategy(BaseTrainer):
    def __init__(self, 
        ...
        # self._collector = ThroughputCollection(**record_config)
...
class DataParallelGroup:
    ...
    def _make_iterator(self, training):
        return [actor.make_iterator.remote(training)
    ...
    def get_data_loader_len(self, training=True):
        lens = ray.get([actor.get_data_loader_len.remote(training) 
                        for actor in self.actors])
    ...
    def validate_batch(self):
        rets = [actor.validate_batch.remote()
                for _, actor in enumerate(self.actors)]
        stats = ray.get(rets)
        return stats
