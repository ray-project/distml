...
class ThroughputCollection(TimerCollection):
    def __init__(
            self,
            batch_size=32,
            # num_workers=1,
        ...
        super(ThroughputCollection, self).__init__()
    ...
    def report(self, key):
        ...
        aggregates[
            f"pass_data_{k}"] = aggregates[f"count_{k}"] * self.batch_size
        aggregates[
            f"throughout_{k}_d"] = aggregates[f"pass_data_{k}"] / t._total_time
