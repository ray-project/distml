...
class JAXTrainingOperator(TrainingOperator):
    ...
    def loss_func(self, params, batch):
        ...
        This function is going to be decorated by `grad` in Jax to calculate gradients.
    ...
    def apply_updates(self, updates):
        ...
        """
        keys, updates = unzip2(sorted(updates.items(), key=lambda d: int(d[0])))
        updates = tree_unflatten(self.opt_state[1], updates)
        self.opt_state = self.opt_update(self.train_step_num, updates, self.opt_state)
    ...
    def to_operator_tensor(self, tensor):
        ...
        There comes a bug. The layouts of tensor explained by cupy and jax are different. But dlpack doesn't convert the layout.
        """
        if isinstance(tensor, list):
